...
int DiscardHoldemBucketer::bucket(std::string key) {
  if (key.size() == 10 || key.size() == 12) {
    return bucketmap[key];
  } else {
    if (key == riverKeyCache1) {
      return riverValCache1;
    ...
    free_results(res);
    res = alloc_results();            
    calc(const_cast<char*>((hand+":"+"6Qs,7Ts,7Js,7Qs,8Ts,8Js,8Qs,9Ts,9Js,9Qs,TJs,T9o,J8o,J9o,JTo,Q8o,Q9o,QTo,QJo").c_str()), const_cast<char*>(board.c_str()), const_cast<char*>(dead.c_str()), 1000000, res);
    point.push_back(*(res->ev));
    free_results(res);
    res = alloc_results();            
    ...
    if (riverCache == 0) {
      riverKeyCache1 = key;
      riverValCache1 = bucket;
      riverCache = 1;
    } else if (riverCache == 1) {
      riverKeyCache2 = key;
      riverValCache2 = bucket;
      riverCache = 0;
    }
    return bucket;
  }
...
int DiscardHoldemBucketer::riverValCache1 = 0;
int DiscardHoldemBucketer::riverValCache2 = 0;
int DiscardHoldemBucketer::riverCache = 0;  
